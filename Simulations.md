...
``` r
dmat <- matrix(0, nrow=length(unique(deltaS$patch1)), ncol=length(unique(deltaS$patch1)))
rownames(dmat) <- colnames(dmat) <- as.character(unique(deltaS$patch1))

for(i in rownames(dmat))
  for(j in colnames(dmat))
    if(length(deltaS$dS[deltaS$patch1 == i & deltaS$patch2 == j]) != 0)
      dmat[i,j] <- dmat[j,i] <- deltaS$dS[deltaS$patch1 == i & deltaS$patch2 == j]
